...
DeepRead is a Chrome Manifest V3 prototype for source-linked reading support. The original webpage remains the reading surface. Reading Spine, Spatial Atlas, Smart Lens, Critical Lens, Explain, and the session-local Reading Trail all connect assistance back to live passages.
...
- **Critical Lens** makes one separate on-demand Gemini request for the current Source Map. It returns zero to three concise questions about `evidence`, `assumption`, `causal`, `uncertainty`, `counterpoint`, or `value`, each citing one or more supplied Source IDs. A finding containing an unknown ID is rejected as a whole. It does not declare the author wrong or act as a fact checker. A ◇ control shows analysing, result count, and an explicit zero state. A compact cached overview, global ◇ Spine positions, X-Ray, and small margin traces connect each question to its original passage. Additional cited passages have navigation links in the expanded trace. Smart and Critical can run independently and stay visible together.
- Atlas visits, opened Smart and Critical findings, and completed Explain interactions can leave up to five Reading Trail traces together. The oldest trace is removed when a sixth is added. Traces are held only in content-script memory and disappear on reload or a Source Map rebuild. They follow their sources while visible, expand on click, and can be dismissed separately. When a new trace or Explain opens, an older expanded trace recedes to its small marker to reduce overlap.
...
- The service worker sends structured-output requests to the single Gemini provider using `gemini-3.5-flash-lite`. Missing keys, API failures, invalid responses, and rate limits show an error instead of fabricated content. Smart and Critical are optional and send sampled page text only when the reader activates their controls; consider those requests when testing private pages or API usage.
...
- Lens controls remain visible in short windows. Both overviews close on scroll and return keyboard focus to their control if needed. Ordinary page-anchor navigation preserves Page Map and Lens caches. Margin markers reserve space for the Spine and its controls; trace details stay inside the viewport and scroll internally if long. Overview labels wrap instead of truncating the question's subject.
...
This prototype does not include translation, follow-up chat, multiple providers, accounts, a production backend, PDF/OCR support, or a settings dashboard.
...
After changing the file, reload the unpacked extension in `chrome://extensions`. Without a key, Explain, Page Map, Smart Lens, and Critical Lens show a missing-key state and do not fabricate output. The model is fixed in `background.js` for this prototype.
...
8. Press **Critical Lens**. Confirm its ◇ control shows analysing, then a persistent count and compact overview; zero findings must remain a valid empty state. Check ◇ positions along the whole Spine, focus a point to see the cited source respond, then choose a finding. The page should navigate to the real passage with Source Spotlight and leave a small Critical trace phrased as a question. Reopen or turn the Lens off and on; an unchanged Source Map should reuse its cached results. Smart ✦ and Critical ◇ should remain distinguishable when both are active.
9. Select text on the original webpage without opening Atlas and choose **Explain**. Confirm the explanation appears beside its passage when there is room. Dismiss it, then expand its Explained trace. The same response should return without another Gemini request. Navigate a Critical or Atlas point and confirm traces can coexist without large overlapping details.
10. Repeat with a narrow Chrome window, a documentation/GitHub-style page, and a dark-background article. Check the controls remain usable, markers do not badly collide, and small DeepRead text stays crisp. An open overview may temporarily overlap a narrow article; close it to continue reading. If testing failure states, temporarily invalidate the local key, reload the extension, and confirm a clear error with live-source points instead of invented AI nodes. Restore the key afterward. For failures, record the page URL, viewport width, Source ID, sanitized console/service-worker error, and screenshot; never share the key.

The 2026-09-26 polish pass passed syntax checks, service-worker contract tests with mocked network responses, and 21 browser-fixture interaction checks on each of light, dark, documentation, and 520 × 360 viewport layouts. A separate zero-result fixture passed its ten applicable checks. The fixture loads the actual UI and source-mapping scripts but mocks extension messaging and Gemini. Native Chrome was detected, but Computer Use stopped because it could not reliably determine its current URL. **No unpacked-extension reload or real-site Chrome acceptance was completed.** The Chrome steps above remain manual acceptance work. The previously documented synthetic Gemini request was not repeated in this pass.
...
Open `http://127.0.0.1:8765/?view=light&autorun=1`, then use `view=dark`, `view=docs`, or `view=zero` for other modes. `/narrow` embeds the dark fixture in a 520 × 360 child viewport. The page reports PASS/FAIL and mock request counts. Reload before rerunning a suite; manual exploration uses the ordinary controls. Tests do not contact Gemini or read the local key. The loopback server serves only an explicit list of fixture/UI assets. See `tests/README.md` for the verification boundary.
...
| `content.js`, `content.css` | Reading Spine, X-Ray peek, light Atlas, Smart and Critical Lens markers, Explain, source navigation, and session Reading Trail. |
